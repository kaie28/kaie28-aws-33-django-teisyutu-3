...
# ★2【自動でSSH作成】→ main.tf#8へ(EC2にその部品を取り付ける)】                                                #2 【(★現在は全ての自動SSHを使用禁止し、手動SSHへ移行)】→ main.tfの#8-6参照
# 2-1 【Terraform(工場)が、自動でpem（カギ)と.pub(鍵穴)を作る。】　　　　　　　　　　　　　　　　　　　　　　　　　　#2-1は使用禁止した。【自動でゼロから「SSHの鍵や鍵穴」を製造】
...
#2-2 自動で　pub「鍵穴」【(public_key)＝★公開鍵　→main.tf#8へ）をEC2に取り付ける】                         　　　#2-2は使用禁止した。【自動でSSHの「鍵穴」をAWSに設置】
...
# 2-3 自動で.pem「鍵」【(private_key）＝★秘密鍵】を裏で保管する。　　　　　　　　　　　　　　　　　　　　　　　　　　#2-3は使用禁止した。【自動でSSHの「鍵」を目隠しながら画面表示させる】
#output "private_key" {
...
resource "aws_iam_instance_profile" "ssm_profile" {                                     #7-3　※#7-1 で作った身分証（ロール）の名前と合致。【全体的】tfvars ➔ variables.tf ➔ main.tf　　
...
#8-6 手動の【SSH鍵穴(ssm-key-kaie28)】を、EC2（kaie28）に取り付ける。　　　　　　　　　　　     #8-6  もしも、SSMが壊れた際の【手動SSH】を準備。　★EC2に手動の「鍵穴」を設置する。
resource "aws_instance" "kaie28" {　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　#【手動「鍵」製造】AWS公式コンソールにて作成  ➔【.pemファイル「鍵」の保管場所】自分のPCで管理
key_name = "ssm-key-kaie28"　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　#【手動「鍵」】tfvars(本物) ➔ variables.tf(SSH用ただの入れ物) ➔ main.tf で「鍵の名前のみTerraformに教えて」紐付けてEC2へ渡し、登録する。
